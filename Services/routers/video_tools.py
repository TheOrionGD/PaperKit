"""Video tools router — delegates all work to the job system."""
...
router = APIRouter(prefix="/tools/video", tags=["video-tools"])

VIDEO_OPS = {
    "convert", "transcode", "trim", "merge",
...
async def run_video_op(
    operation: str,
    body: dict,
    current_user: dict = Depends(get_current_user),
):
    if operation not in VIDEO_OPS:
    ...
    job = await job_service.create_job(
        db, user_id, f"video.{operation}", input_assets, parameters
    )
    return job
